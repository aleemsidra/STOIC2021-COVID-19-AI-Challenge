...
config_file = process_config(os.path.join('/image_classification_pytorch', 'configs', 'config.json'))
class ExampleLogger:
    """
    ...
    def init(self):
        ...
        self.log_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', self.config['model_net_name'])
        self.model_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', )

        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)
        
        if not os.path.exists(os.path.join (self.log_dir, self.config['model_net_name'])):
             self.model_dir = os.path.join(self.log_dir, self.config['model_net_name'] )
