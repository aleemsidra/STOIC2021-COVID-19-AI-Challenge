...
class BaseTrainer:
    def __init__(self, model, train_loader, val_loader, config, logger, time, exp_suffix='random', wandb_mode='disabled'):
        self.model = model
        self.logger = logger
        self.config = config
        ...
        self.save_path = self.config['model_net_name'] + "_" +exp_suffix+"_"+ self.time
        self.wandb_run = wandb.init(project='stoic', entity='mayug', name=self.config['model_net_name'] + "_" +exp_suffix+"_"+ self.time, mode=wandb_mode)
    ...
    def train(self):
        ...
        #print('epochs' , total_epoch_num)
        if self.config['evaluate_before_train']:
            
            self.evaluate_epoch()
            self.eval_validate = self.eval_top1.value()
            print('\n')
        for cur_epoch in range(1, total_epoch_num+1):
            ...
            print('best_acc so far ', [best_acc, best_acc_epoch])
            print('best_auc so far ', [best_auc, best_auc_epoch])
